...
function deplacement(element,position){
    ...
    else if(x>window.innerWidth && y>window.innerHeight){
...
function agrandirLereprere(){
    if((window.innerWidth<500 || window.innerHeight<500)&& id>10){
        return 2;
    }
    if((window.innerWidth>1200 || window.innerHeight>600)&& id>25){
        return 1.5;
    }
}
function main (){
...
class Postit{
    ...
    ajouterPostit(){
        ...
        var post_it = {
            ...
            type:'div',
            text:text,
            axeX : getRandomIntInclusive(0,window.innerWidth),
            axeY: getRandomIntInclusive(0,window.innerHeight),
            distance: 0,
            angleX:0,
            couleur:randomColor()
        };
        ...
        while(verifierLacollision(post_it.axeX,post_it.axeY,tableaudepostit)!=1){
            let taux=agrandirLereprere();
            post_it.axeX=getRandomIntInclusive((-window.innerWidth)*taux,window.innerWidth*taux);
            post_it.axeY=getRandomIntInclusive((-window.innerHeight)*taux,window.innerHeight*taux);
